...
class Person: NSObject {
    ...
    /// 插入
    func insertSQL(_ isSuccess: (Bool)->()){
        ...
        //3、执行语句
        let success = SQLManager.shareInstance().execSQL(sql: sql)
        isSuccess(success)
    ...
    /// 删除
    func deleteSQL(_ isSuccess: (Bool)->()){
    ...
    class func queryLimitPerson(m : Int, n : Int) -> [Person]{
        if n == 0 {
            return loadPersons()
        }
